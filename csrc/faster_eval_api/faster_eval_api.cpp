...
#error "GCC >= 4.9 is required!"
    ...
    PYBIND11_MODULE(faster_eval_api_cpp, m)
    {
        ...
        m.def("get_compiler_version", &get_compiler_version, "Returns the compiler version used for compilation.");
        m.def("COCOevalAccumulate", &COCOeval::Accumulate, "Accumulates evaluation statistics.");
        m.def("COCOevalEvaluateImages", &COCOeval::EvaluateImages, "Evaluates images based on detections and ground truth.");
        m.def("COCOevalEvaluateAccumulate", &COCOeval::EvaluateAccumulate, "Performs evaluation and accumulation in one step.");
        m.def("calc_auc", &COCOeval::calc_auc, "Calculates area under curve (AUC) for PR curve.");
        ...
        pybind11::class_<COCOeval::ImageEvaluation>(m, "ImageEvaluation")
            ...
            .def(pybind11::pickle(
                ...
                {
                    // Use reserve to avoid reallocations (performance optimization)
                    std::vector<std::tuple<uint64_t, uint64_t, double>> matched_annotations;
                    matched_annotations.reserve(p.matched_annotations.size());
                    for (const auto &ann : p.matched_annotations)
                    {
                        matched_annotations.emplace_back(ann.dt_id, ann.gt_id, ann.iou);
                    }
                    return pybind11::make_tuple(
                        p.detection_matches,
                        p.ground_truth_matches,
                        p.detection_scores,
                        p.ground_truth_ignores,
                        p.detection_ignores,
                        matched_annotations);
                },
                ...
                {
                    if (t.size() != 6)
                        throw std::runtime_error("Invalid state for ImageEvaluation!");
                    ...
                    p.matched_annotations.reserve(matched_annotations.size());
                    for (const auto &tup : matched_annotations)
                    {
                        p.matched_annotations.emplace_back(std::get<0>(tup), std::get<1>(tup), std::get<2>(tup));
                    }
                    return p;
                }));
        ...
        pybind11::class_<COCOeval::Dataset>(m, "Dataset")
            ...
            .def(pybind11::pickle(
                ...
                {
                    // Recommend: Ensure make_tuple() is efficient and returns minimal necessary state.
                    return p.make_tuple();
                },
                [](pybind11::tuple t)
                {
