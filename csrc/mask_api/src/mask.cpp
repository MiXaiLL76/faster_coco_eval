...
namespace mask_api
{
    ...
    namespace Mask
    {
        ...
        // Encodes a 3D numpy array (py::array_t<uint8_t, py::array::f_style>) into a vector of RLE (Run-Length Encoding) objects.
        // Parameters:
        //   M - A 3D numpy array with dimensions [height, width, n], containing uint8_t mask data.
        //   h - The height of the mask (number of rows).
        //   w - The width of the mask (number of columns).
        //   n - The number of masks (third dimension of M).
        //
        // Returns:
        //   A std::vector of RLE objects, each representing the run-length encoding of a single mask.
        //
        // The function iterates through each mask (along the third dimension), and for each mask,
        // it traverses all elements in column-major order. It counts consecutive runs of identical values
        // and stores the counts in a vector, which is then used to construct an RLE object.
        std::vector<RLE> rleEncode(const py::array_t<uint8_t, py::array::f_style> &M, uint64_t h, uint64_t w, uint64_t n)
        {
            ...
            for (uint64_t i = 0; i < n; ++i)
            {
                ...
                cnts.reserve(h * w / 2); // Reserve estimated size to avoid reallocations
        ...
        // Converts a vector of RLE (Run-Length Encoding) objects to a 2D NumPy array of bounding boxes.
        // Parameters:
        //   R - A std::vector of RLE objects. Each RLE object should provide a toBbox() method that returns a vector of 4 unsigned integers
        //       representing the bounding box in the format [x_min, y_min, width, height].
        //   n - (optional) The number of RLE elements to process. If not provided, all elements in R are processed.
        //
        // Returns:
        //   A py::array NumPy array of shape [n, 4], where each row corresponds to the bounding box of a mask.
        //
        // The function processes up to n RLE objects (or all if n is not specified). For each RLE object, it extracts the bounding box
        // using the toBbox() method and appends the results to a flat std::vector<double>. Finally, the function returns a NumPy array
        // of shape [n, 4] containing all bounding boxes.
        py::array rleToBbox(const std::vector<RLE> &R, std::optional<uint64_t> n)
        {
            ...
            return py::array(
                py::buffer_info(
                    ...
                    py::format_descriptor<double>::format(),                       // Python struct-style format descriptor (const char*)
                    2,                                                             // Number of dimensions
                    std::vector<ssize_t>{static_cast<ssize_t>(count), 4},          // Shape
                    std::vector<ssize_t>{static_cast<ssize_t>(4 * sizeof(double)), // Strides (row stride)
                                         static_cast<ssize_t>(sizeof(double))}     // Strides (column stride)
                    ));
        }
        ...
        // Converts a flat vector of bounding box coordinates to a vector of RLE (Run-Length Encoding) objects representing binary masks.
        // Parameters:
        //   bb - A std::vector<double> containing bounding box coordinates. Each bounding box is represented by 4 consecutive values:
        ...
        //
        // Returns:
        //   A std::vector<RLE> where each element is an RLE object corresponding to one bounding box, encoded as a binary mask
        //   using the given height and width.
        //
        // The function checks if the input vector bb has at least n*4 elements. For each bounding box, it extracts its four coordinates,
        // constructs a bounding box array, and passes it to RLE::frBbox to create the RLE mask. All resulting RLE objects are collected
        // in a std::vector<RLE> which is returned to the caller. Throws std::invalid_argument if the input vector is too small.
        std::vector<RLE> rleFrBbox(const std::vector<double> &bb, uint64_t h, uint64_t w, uint64_t n)
        {
            ...
            if (bb.size() < n * 4)
            {
                throw std::invalid_argument("Input vector bb has insufficient size");
            }
        ...
        // Converts a vector of RLE objects to a vector of Python dicts using RLE::toDict().
        ...
        // Converts a set of RLE-encoded binary masks into a single contiguous 3D array, where each mask occupies a separate slice
        // along the third axis. Throws an exception if the encoding is invalid or exceeds the expected output dimensions.
        //
        // Parameters:
        //  - const std::vector<RLE>& R: A vector of RLE structures, each representing a binary mask with specific height (h),
        //    width (w), a count of RLE pairs (m), and a vector of run counts (cnts).
        //
        // Returns:
        //  - py::array_t<uint8_t, py::array::f_style>: A 3D numpy array with shape [h, w, n], where n is the number of masks.
        //    Each mask is stored as a 2D binary array in the output along the third dimension.
        //
        // Usage:
        ...
        py::array_t<uint8_t, py::array::f_style> rleDecode(const std::vector<RLE> &R)
        {
            ...
            py::array_t<uint8_t, py::array::f_style> M({static_cast<size_t>(h), static_cast<size_t>(w), n});
        ...
        std::vector<py::dict> frUncompressedRLE(const std::vector<py::dict> &ucRles)
        {
            ...
            for (uint64_t i = 0; i < ucRles.size(); i++)
            {
                ...
                std::vector<uint64_t> counts = ucRles[i]["counts"].cast<std::vector<uint64_t>>();
        ...
        //   - gt: vector of doubles, concatenated ground-truth boxes, each as [x, y, w, h]
        //   - m: number of predicted (dt) boxes
        //   - n: number of ground-truth (gt) boxes
        //   - iscrowd: vector of int, with n entries; if iscrowd[g] is set, special crowd IoU computation is used
        //
        // Returns:
        //   - vector<double>: m*n IoU values between every dt and gt box (row-major: o[d*n + g])
        std::vector<double> bbIou(
            ...
            const std::vector<int> &iscrowd)
        {
            ...
            if (dt.size() != m * 4 || gt.size() != n * 4)
                throw std::invalid_argument("Input box vector size does not match m or n.");

            if (!iscrowd.empty() && iscrowd.size() != n)
                throw std::invalid_argument("iscrowd size must be 0 or equal to n.");
            ...
            for (std::size_t g = 0; g < n; ++g)
            {
                ...
                for (std::size_t d = 0; d < m; ++d)
                {
                    ...
                    const double intersect_w = std::min(dx + dw, gx + gw) - std::max(dx, gx);
                    if (intersect_w <= 0.0)
                        continue;
                    const double intersect_h = std::min(dy + dh, gy + gh) - std::max(dy, gy);
                    if (intersect_h <= 0.0)
                        continue;
        ...
        // Converts a Python object representing bounding boxes into a flat 1D vector of doubles.
        // Parameters:
        ...
        // Preprocesses a Python object into a flat vector of bounding boxes or a vector of RLEs.
        // Parameters:
        //   - pyobj: Python object, can be numpy.ndarray (boxes), list of lists (boxes), or list of dicts (RLEs).
        // Returns:
        //   - Tuple containing:
        ...
        std::tuple<std::variant<std::vector<RLE>, std::vector<double>>, size_t> _preproc(const py::object &pyobj)
        {
            ...
            else if (type == "<class 'list'>")
            {
                ...
                {
                    auto matrix = pyobj.cast<std::vector<std::vector<double>>>();
            ...
            check_rle:
                ...
                {
                    auto result = _frString(pyobj.cast<std::vector<py::dict>>());
                    return {result, result.size()};
                }
                throw std::out_of_range("list input can be bounding box (Nx4) or RLEs ([RLE])");
            }
            throw std::out_of_range("unrecognized type. Supported types: RLEs (rle), np.ndarray (box), and list (box/RLE).");
        }

        // Computes the Intersection over Union (IoU) between detections and ground truths.
        // Supports both bounding box and RLE inputs, and checks input compatibility.
        // Parameters:
        ...
        // Returns:
        //   - 2D numpy array (shape: [m, n]) of IoU values, where m=number of detections and n=number of gt objects.
        //   - If no matches, returns an empty vector<double>.
        // Throws:
        //   - std::out_of_range if types differ or iscrowd length mismatches gt count.
        std::variant<py::array_t<double, py::array::f_style>, std::vector<double>>
        iou(const py::object &dt, const py::object &gt, const std::vector<int> &iscrowd)
        {
        ...
        // Converts a Python object representing segmentation data into COCO RLE or polygon/bbox encodings as pybind11 dicts.
        // Handles lists, numpy arrays, and dicts, and dispatches to the appropriate encoding function based on the object type and shape.
        // Parameters:
        //   - pyobj: Python object representing the input segmentation (could be a list, numpy.ndarray, or dict).
        //   - h: Image height (uint64_t), used for encoding.
        //   - w: Image width (uint64_t), used for encoding.
        // Returns:
        //   - If input is a list of objects, returns a vector of pybind11::dict representing encoded masks or polygons.
        //   - If input is a single object (dict or bbox/poly array), returns a single pybind11::dict representing the encoded mask or polygon.
        // Throws:
        ...
        std::variant<pybind11::dict, std::vector<pybind11::dict>> frPyObjects(const py::object &pyobj, const uint64_t &h, const uint64_t &w)
        {
            ...
            if (type == "<class 'list'>")
            {
                ...
                // List of lists or numpy arrays: treat as bbox or polygon depending on shape
                ...
                // List of floats or ints: treat as a single bbox or polygon depending on length
                else if ((sub_type == "<class 'float'>") || (sub_type == "<class 'int'>"))
                {
                    std::vector<double> array = pyobj.cast<std::vector<double>>();
                    if (array.size() == 4)
                    {
                        return frBbox({array}, h, w)[0]; // Return the first (and only) dict
                    }
                    else if (array.size() > 4)
                    {
                        return frPoly({array}, h, w)[0]; // Return the first (and only) dict
                    }
                }
            }
            ...
            {
                return frUncompressedRLE({pyobj})[0]; // Return the first (and only) dict
            }
            else
            {
        ...
        // Converts a segmentation Python object to a COCO RLE (Run-Length Encoding) dictionary.
        // If conversion is not possible due to a type error, returns the original Python object.
        // Parameters:
        //   - pyobj: Python object representing the segmentation mask (polygon, uncompressed RLE, or other compatible format).
        //   - w: Image width (uint64_t), used for encoding.
        //   - h: Image height (uint64_t), used for encoding.
        // Returns:
        //   - pybind11::dict containing the RLE-encoded mask if conversion succeeds.
        //   - Otherwise, returns the original py::object if conversion fails with py::type_error.
        ...
        // Calculates and adds RLE masks (and optionally boundary RLEs) for all annotation objects in the input list.
        // For each annotation with a "segmentation" field, computes the RLE mask using its associated image size.
        // If boundary computation is enabled, also computes the boundary RLE for each annotation in parallel.
        // The function updates each annotation in place by adding "rle" and, if requested, "boundary" fields.
        // Parameters:
        //   - anns: Vector of py::dict annotation objects, each representing an instance annotation.
        //   - image_info: Map from image_id to a tuple (height, width) for each image.
        //   - compute_rle: If true, computes and assigns the RLE mask to each annotation.
        //   - compute_boundary: If true, computes and assigns the boundary RLE to each annotation.
        //   - dilation_ratio: Ratio of the mask diagonal for the erosion radius in boundary extraction.
        //   - cpu_count: Number of CPU threads to use for parallel boundary computation.
        // Returns:
