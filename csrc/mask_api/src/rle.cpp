    ...
    namespace Mask
    {
        ...
        std::string RLE::toString() const
        {
            ...
            for (std::size_t i = 0; i < m; ++i)
            {
                ...
                while (more)
                {
                    ...
                    x >>= 5;
                    // If the sign bit (0x10) is set, continue if x != -1; otherwise, continue if x != 0
                    more = (c & 0x10) ? x != -1 : x != 0;
                    if (more)
        ...
        RLE RLE::frString(const std::string &s, uint64_t h, uint64_t w)
        {
            ...
            while (i < m)
            {
                ...
                {
                    if (i >= m)
                        throw std::runtime_error("RLE string is malformed: early end.");

                    // Subtract '0' to get value in range [0..63]
                    const int64_t c = static_cast<int64_t>(s[i]) - static_cast<int64_t>('0');
                    x |= (c & 0x1f) << (5 * k);
                    more = c & 0x20;
                    ++i;
                    ++k;

                    // If highest bit of this chunk is set and this is the last chunk, extend sign
                    if (!more && (c & 0x10))
                    {
                        x |= (~0LL) << (5 * k);
                    }
        ...
        // Returns the bounding box [x, y, w, h] for the RLE mask as a vector of doubles.
        // The bounding box tightly encloses all foreground pixels in the mask.
        std::vector<double> RLE::toBbox() const
        {
            ...
            uint64_t h = static_cast<uint64_t>(this->h), w = static_cast<uint64_t>(this->w);
            ...
            for (size_t j = 0; j < m; ++j)
            {
                ...
                cc += this->cnts[j]; // End index (exclusive) of current segment
        ...
        // Converts a polygon (represented by a list of (x, y) pairs) into an RLE mask representation.
        // Parameters:
        //   - xy: vector of double, length 2*k, representing (x, y) coordinates of polygon vertices
        //   - h: mask height
        //   - w: mask width
        // Returns:
        ...
        RLE RLE::frPoly(const std::vector<double> &xy, uint64_t h, uint64_t w)
        {
            ...
            for (j = 0; j < k; ++j)
            {
                ...
                if (dx >= dy)
                {
                    for (int d = 0; d <= dx; ++d)
                    {
                        t = flip ? dx - d : d;
                        u.emplace_back(t + xs);
                        v.emplace_back(static_cast<int>(ys + s * t + 0.5));
                    }
                }
                else
                {
                    for (int d = 0; d <= dy; ++d)
                    {
                        t = flip ? dy - d : d;
                        v.emplace_back(t + ys);
                        u.emplace_back(static_cast<int>(xs + s * t + 0.5));
                    }
                }
            }
            ...
            for (std::size_t j = 1; j < u.size(); ++j)
            {
                if (u[j] != u[j - 1])
                {
                    ...
                    xd = (xd + 0.5) / scale - 0.5;
                    if ((!AreEqual(std::floor(xd), xd)) || xd < 0 || xd > static_cast<double>(w - 1))
                    {
                        continue;
                    }
            ...
            for (std::size_t j = 0; j < x.size(); ++j)
                a.emplace_back(static_cast<uint32_t>(x[j] * static_cast<int>(h) + y[j]));
        ...
        // Parameters:
        //   - bb: vector of 4 doubles, [x, y, width, height], representing the bounding box
        //   - h: mask height
        //   - w: mask width
        // Returns:
        ...
        // Performs morphological erosion with a 3x3 (or arbitrary dilation) structuring element on the binary mask represented by RLE.
        // Parameters:
        ...
        RLE RLE::erode_3x3(int dilation) const
        {
            ...
            // Prepare offsets for the structuring element (above and on the current pixel)
            ...
            for (int i = dilation; i >= 0; i--)
            {
                for (int j = dilation; j >= -dilation; j--)
                {
                    if (i == 0 && j <= 0)
                        continue;
                    if (i > 0)
                        ofsvec.push_back(i * static_cast<int>(this->h) + j);
                    else
                        ofsvec.push_back(j);
                }
            }
            ...
            {
                ofsvec_bottom.push_back(i * static_cast<int>(this->h) + dilation);
            }
            ...
            for (uint64_t j : this->cnts)
            {
                ...
                if (v)
                {
                    ...
                    for (uint64_t k = 0; k < j; k++)
                    {
                        ...
                        if (_prev_min)
                        {
                            _min = std::all_of(ofsvec_bottom.begin(), ofsvec_bottom.end(),
                                               [c, max_len, rle_h, &mask, y, dilation](int o)
                                               {
                                                   long test_ptr = c + o;
                                                   return (test_ptr >= 0) &&
                                                          (test_ptr < max_len) &&
                                                          mask[test_ptr] &&
                                                          (std::abs((test_ptr % rle_h) - y) <= dilation);
                                               });
                        }
                        else
                        {
                            _min = std::all_of(ofsvec.begin(), ofsvec.end(),
                                               [c, max_len, rle_h, &mask, y, dilation](int o)
                                               {
                                                   long test_ptr = c + o;
                                                   long test_ptr_mirror = c - o;
                                                   return (test_ptr_mirror >= 0) && mask[test_ptr_mirror] &&
                                                          (test_ptr < max_len) && mask[test_ptr] &&
                                                          (std::abs((test_ptr % rle_h) - y) <= dilation);
                                               });
                        }
        ...
        // Removes zero runs and merges consecutive duplicate runs in the RLE counts vector.
        RLE RLE::clear_duplicates() const
        {
            ...
            for (std::size_t i = 0; i < this->cnts.size(); ++i)
            {
                if (i > 0)
                {
                    ...
                    {
                        // Merge zero and consecutive runs into the previous run
        ...
        // Returns the boundary of the mask as a new RLE, using morphological erosion and XOR.
        // Parameters:
        //   - dilation_ratio: ratio of mask diagonal for erosion radius (typical: 0.008)
        // Returns:
        //   - RLE mask representing the boundary pixels
        RLE RLE::toBoundary(double dilation_ratio) const
        {
            // Compute dilation size as a function of mask diagonal and requested ratio
            int dilation = static_cast<int>(std::round(dilation_ratio * std::sqrt(this->h * this->h + this->w * this->w) - 1e-10));
            if (dilation < 1)
            {
                dilation = 1;
            }
        ...
        // Returns the total area (number of foreground pixels) represented by the RLE mask.
        // Only the odd-indexed runs (foreground) are summed.
        uint64_t RLE::area() const
        {
            uint64_t result = 0;
            for (std::size_t j = 1; j < this->m; j += 2)
                result += this->cnts[j];
            return result;
        }
        ...
        {
            return RLE::frString(
                std::get<2>(w_h_rlestring), // rle_string
        ...
        RLE RLE::frUncompressedRLE(const pybind11::dict &ucRle)
        {
            ...
            std::vector<uint64_t> counts = ucRle["counts"].cast<std::vector<uint64_t>>();
        ...
        RLE RLE::frSegm(const pybind11::object &pyobj, uint64_t w, uint64_t h)
        {
            ...
            {
                std::string sub_type = py::str(py::type::of(pyobj.attr("get")("counts")));
                ...
                {
                    throw py::type_error("counts type must be list, bytes, or str for RLE");
                }
            }
            else
            {
                throw py::type_error("Segm type must be list (polygon) or dict (RLE)");
            }
        }
    } // namespace Mask
