...
//
// Python C++ extension module for efficient mask encoding, decoding, and manipulation using pybind11.
// Provides the RLE (Run-Length Encoding) class and a comprehensive set of mask operations for computer vision pipelines.
//
// Features:
//   - RLE encoding/decoding for binary masks, polygons, and bounding boxes
//   - Morphological operations (erosion, boundary extraction)
//   - Conversion between different mask representations (RLE, polygon, bbox, uncompressed)
//   - Calculation of mask area, bounding boxes, and IoU (Intersection over Union) metrics
...
//   boundary = rle.toBoundary(0.008)  # get boundary mask with given dilation ratio
...
namespace mask_api
{
  ...
  // Parameters: none
  // Returns:
  //   - String indicating the compiler and version (e.g., "GCC 9.3", "clang 12.0.0", "MSVC 1928")
...
#error "GCC >= 4.9 is required!"
  ...
  PYBIND11_MODULE(mask_api_new_cpp, m)
  {
    ...
    pybind11::class_<Mask::RLE>(m, "RLE")
        .def(pybind11::init<uint64_t, uint64_t, uint64_t, std::vector<uint64_t>>())
    ...
    // Module-level utility functions for mask encoding, decoding, and manipulation.
    ...
    // Computes intersection-over-union (IoU) between RLE masks or bounding boxes.
    m.def("iou", &Mask::iou, "Mask::iou");
    m.def("bbIou", &Mask::bbIou, "Mask::bbIou");
    m.def("rleIou", &Mask::rleIou, "Mask::rleIou");

    // Converts RLE, polygons, or bounding boxes to the respective representations.
    ...
    // Batch utility to compute and attach RLEs and boundaries to annotation objects.
