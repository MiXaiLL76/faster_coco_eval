...
class FasterCOCODetection(COCODetection):
    def __init__(
        ...
        **kwargs,
        ...
        if not HAS_FASTER_COCOAPI:
            raise RuntimeError(
                "Failed to import `COCO` and `COCOeval` from "
        ...
        if ann_file is not None:
            with get_local_path(
                filepath=ann_file, backend_args=backend_args
            ) as local_path:
    ...
    def gt_to_coco_json(
        ...
        categories = [dict(id=id, name=name) for id, name in enumerate(classes)]
        image_infos: list = []
        annotations: list = []
    ...
    def compute_metric(self, results: list) -> dict:
        ...
        if len(self.cat_ids) == 0:
            self.cat_ids = self._coco_api.get_cat_ids(cat_names=classes)  # type: ignore
        ...
        for metric in self.metrics:
            ...
            coco_eval = COCOeval(
                self._coco_api, coco_dt, iou_type, extra_calc=self.extra_calc
            )
        ...
        for metric in self.metrics:
            ...
            if metric == "proposal":
                ...
                if self.metric_items is None:
                    ...
                    headers = [f"{metric}_{item}" for item in self.metric_items]
