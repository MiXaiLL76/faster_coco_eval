...
from faster_coco_eval import COCO as COCO_faster
from faster_coco_eval import COCOeval_faster

if "CocoMetric" in METRICS.module_dict:
    del METRICS.module_dict["CocoMetric"]
...
class CocoMetric(BaseMetric):
    ...
    def __init__(
        ...
        if ann_file is not None:
            with get_local_path(
                ...
                if self.use_faster:
                    self._coco_api = COCO_faster(local_path)
                else:
    ...
    def compute_metrics(self, results: list) -> Dict[str, float]:
        ...
        if self._coco_api is None:
            ...
            if self.use_faster:
                self._coco_api = COCO_faster(coco_json_path)
            else:
            ...
            if self.use_mp_eval:
                ...
                if self.use_faster:
                    coco_eval = COCOeval_faster(
                        self._coco_api, coco_dt, iou_type, print_function=print
                    )
                else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("tmp_results", type=str, help="path to tmp_results.pkl")
    parser.add_argument(
        "dataset_meta", type=str, help="path to dataset_meta.pkl"
    )
    parser.add_argument("--metric", type=str, help="metric", default="bbox")
    parser.add_argument(
        "--use_faster", action=argparse.BooleanOptionalAction, default=False
    )

    args = parser.parse_args()

    with open(args.tmp_results, "rb") as fd:
        tmp_results = pickle.load(fd)

    with open(args.dataset_meta, "rb") as fd:
        dataset_meta = pickle.load(fd)

    print(args)

    metric = CocoMetric(metric=args.metric, use_faster=args.use_faster)
    metric.dataset_meta = dataset_meta
    metric.results = tmp_results

    ts = time.time()
    metric.evaluate(len(tmp_results))
    te = time.time()

    print(f"Total evaluate: {te - ts:.2f}s")
